...
2. vexset[i]:标识各个顶点所属的连通分量. 对于每个含于V中的vi,在辅助数组中存在一个相应元素vexset[i]表示该顶点所在的连通分量
...
​	此算法依赖Edge的排序， 若使用堆排序，排序时间为O(elog_2e). 在循环中最好是的操作是合并两个不同的连通分量， 只要采取合适的数据结构， 可以证明其执行时间为O(log_2e),整个算法的时间复杂度为O(elog_2e),与边数有关，所以更适合求稀疏王的最小生成树